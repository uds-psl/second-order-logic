...
Section ND_def.
  ...
  | AllI {ff} {p} A phi : List.map (subst_form ↑) A ⊢ phi -> A ⊢ ∀ phi
  ...
  Notation "A ⊢CE phi" := (@prv falsity_on class A phi) (at level 30).
  Notation "A ⊢CL phi" := (@prv falsity_off class A phi) (at level 30).
  Notation "A ⊢IE phi" := (@prv falsity_on intu A phi) (at level 30).

  Definition tprv {ff} {p} T phi := (exists A, A ⊏ T /\ @prv ff p A phi).

  Notation "T ⊩ phi" := (tprv T phi) (at level 30).
  Notation "T '⊩(' s , b ')' phi" := (@tprv s b T phi)  (at level 30).
  Notation "T ⊩CE phi" := (@tprv falsity_on class T phi) (at level 30).
  Notation "T ⊩CL phi" := (@tprv falsity_off class T phi) (at level 30).
  Notation "T ⊩IE phi" := (@tprv falsity_on intu T phi) (at level 30).
  ...
  Lemma cycle_shift_subject n phi :
    unused (S n) phi -> phi[(var n)..][cycle_shift n] = phi.
  Proof.
    intros H. rewrite subst_comp. erewrite (subst_unused_single H); try now apply subst_id.
    intros m H'; unfold funcomp; comp. cbn. destruct m; cbn.
    - decide (n = n); trivial. congruence.
    - decide (n = m); trivial. congruence.
  Qed.

  Lemma nameless_equiv A phi n :
    unused_L n A -> unused (S n) phi -> ((A ⊢ phi[(var n)..]) <-> [phi[↑] | phi ∈ A] ⊢ phi).
  Proof.
    intros HL Hphi. split.
    - intros H % (subst_Weak (cycle_shift n)).
      rewrite cycle_shift_subject, (map_ext_in _ (subst_form ↑)) in H.
      1,3: assumption. intros ? ? % HL. now apply cycle_shift_shift.
    - intros H % (subst_Weak (($n)..)). rewrite map_map in *. rewrite (map_ext _ id), map_id in H.
      assumption. intuition comp. erewrite subst_comp. apply subst_id. now intros [].
  Qed.
  End Weakening.

  Lemma prv_cut {p : peirce} {ff : falsity_flag} A phi psi :
    A ⊢ phi -> (phi :: A) ⊢ psi -> A ⊢ psi.
  Proof.
    eauto.
  Qed.

  Lemma tprv_list_T {p : peirce} {ff : falsity_flag} A phi :
    list_T A ⊩ phi -> A ⊢ phi.
  Proof.
    intros (B & HB1 & HB2). apply (Weak HB2). firstorder.
  Qed.

  Definition capture_subs n x := var (x + n).

  Lemma capture_extract {p : peirce} {ff : falsity_flag} n A phi :
  ...
  Lemma close_extract {p : peirce} {ff : falsity_flag} A phi :
    A ⊢ close phi -> A ⊢ phi.
  Proof.
  ...
  Lemma big_imp_extract {p : peirce} {ff : falsity_flag} A B phi :
  ...
  Lemma prv_from_single {p : peirce} {ff : falsity_flag} A phi :
...
Notation "A ⊢IE phi" := (@prv _ _ falsity_on intu A phi) (at level 30).

Arguments tprv {_ _ _ _} _ _.
Notation "T ⊩ phi" := (tprv T phi) (at level 30).
Notation "T '⊩(' s , b ')' phi" := (@tprv _ _ s b T phi)  (at level 30).
Notation "T ⊩CE phi" := (@tprv _ _ falsity_on class T phi) (at level 30).
Notation "T ⊩CL phi" := (@tprv _ _ falsity_off class T phi) (at level 30).
Notation "T ⊩IE phi" := (@tprv _ _ falsity_on intu T phi) (at level 30).
...
Ltac ospecialize n t := 
  eapply prv_cut; [eapply (@AllE _ _ _ _ _ t); ctx_index n|]; cbn.
