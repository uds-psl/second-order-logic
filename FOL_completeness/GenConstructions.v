...
Section GenCons.
  ...
  Notation "⊥" := GBot.
  Notation "A ⊢G phi" := (@prv Σf Σp b class A phi) (at level 30).
  Notation "T ⊩G phi" := (@tprv Σf Σp b class T phi) (at level 30).
  ...
  Section Explosion.
    ...
    Proof.
      ...
      apply subst_unused_closed' with (xi := ↑) (phi0 := exp_axiom phi).
  ...
  Ltac clean H := rewrite! (subst_unused_closed' _ H).
  ...
  Ltac odestruct n := eapply GExE; [intuition|ctx_index n|]; cbn.

  Lemma GExI A t phi :
    A ⊢G phi [t .: var] -> A ⊢G ∃ phi.
  Proof.
    intros Hc. apply II.
