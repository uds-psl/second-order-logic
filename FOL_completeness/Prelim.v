...
Notation "'eq_dec2' X" := (forall x y : X, dec (x=y)) (at level 70).

Structure eqType := EqType {
  eqType_X :> Type;
  eqType_dec : eq_dec2 eqType_X }.

Arguments EqType X {_} : rename.

Canonical Structure eqType_CS X (A: eq_dec2 X) := EqType X.

Existing Instance eqType_dec.

Instance unit_eq_dec2 :
  eq_dec2 unit.
Proof.
  unfold dec. decide equality. 
Qed.

Instance bool_eq_dec2 : 
  eq_dec2 bool.
Proof.
  unfold dec. decide equality. 
Defined.

Instance nat_eq_dec2 : 
  eq_dec2 nat.
Proof.
  unfold dec. decide equality.
Defined.

Instance prod_eq_dec2 X Y :  
  eq_dec2 X -> eq_dec2 Y -> eq_dec2 (X * Y).
Proof.
  unfold dec. decide equality. 
Defined.

Instance list_eq_dec2 X :  
  eq_dec2 X -> eq_dec2 (list X).
Proof.
  unfold dec. decide equality. 
Defined.

Instance sum_eq_dec2 X Y :  
  eq_dec2 X -> eq_dec2 Y -> eq_dec2 (X + Y).
Proof.
  unfold dec. decide equality. 
Defined.

Instance option_eq_dec2 X :
  eq_dec2 X -> eq_dec2 (option X).
Proof.
  unfold dec. decide equality.
Defined.

Instance Empty_set_eq_dec2:
  eq_dec2 Empty_set.
Proof.
  unfold dec. decide equality.
Qed.

Instance True_eq_dec2:
  eq_dec2 True.
Proof.
  intros x y. destruct x,y. now left.
Qed.

Instance False_eq_dec2:
  eq_dec2 False.
Proof.
  intros [].
Qed.
